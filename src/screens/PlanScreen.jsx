...
const PlanScreen = ({ kickoff, clearKickoff, setRoute, trip }) => {
  ...
  const applyMode = (m) => {
    ...
    setChat(c => [...c, { who: 'voya', text: `Apliquei o modo “${m.label}” ao seu roteiro. ${m.delta ? `(${m.delta})` : ''} Posso ajustar mais alguma coisa?` }]);
  };
  ...
  return (
    <div className="grid grid-cols-[400px_1fr] xl:grid-cols-[440px_1fr] h-screen sticky top-0">
      ...
      <section className="border-r hairline flex flex-col bg-paper min-h-0">
        <header className="px-7 pt-6 pb-4 border-b hairline bg-paper">
          <div className="label mb-1">Concierge IA · com expert Inês</div>
          <div className="flex items-center justify-between">
            <h2 className="text-[20px] font-medium tracking-tight text-ink-900">Conversa</h2>
            <div className="flex items-center gap-2">
              <Tag tone="sage"><span className="h-1.5 w-1.5 rounded-full bg-sage-500"/> Inês online</Tag>
  ...
  return (
    <div className="px-8 pt-7 pb-5 border-b hairline bg-paper">
      <div className="flex items-start gap-5">
        <SmartImg seed={trip.coverSeed} tone={trip.cover} label={trip.coverLabel} w={240} h={180} className="h-[88px] w-[120px] rounded-xl shrink-0"/>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              <div className="label mb-1">Roteiro vivo</div>
              <h1 className="text-[24px] tracking-tight font-medium text-ink-900 leading-tight">{trip.title}</h1>
              <p className="text-[13px] text-ink-600 mt-1">{trip.blurb}</p>
            </div>
    ...
    <Drawer
      ...
      <div className="px-6 py-5 border-b hairline">
        ...
        <div className="mt-3 text-[12.5px] text-ink-600 leading-relaxed">
          {item.conf
            ? <span className="inline-flex items-center gap-1.5"><Icon.Check size={12} className="text-sage-700"/> Reserva confirmada — para mudar, a Voya pode liberar.</span>
      ...
      {tab === 'alts' ? (
        <div className="px-6 py-5 space-y-3">
          <div className="label">A Voya sugere para você</div>
          <div className="space-y-2.5">
            ...
            {alts.length === 0 && (
              <div className="bg-ink-50 rounded-xl p-4 text-[13px] text-ink-600">
                Nada nessa busca. Pode pedir algo específico no chat — a Voya costuma achar.
              </div>
            )}
          </div>
          ...
          <div className="pt-5 mt-3 border-t hairline">
            ...
            <button className="w-full bg-white border-half rounded-xl px-4 py-3 flex items-center gap-3 hover:border-ink-400 transition-colors text-left">
              ...
              <div className="flex-1">
                ...
                <div className="text-[12px] text-ink-500">Descreva no chat e a Voya monta a alternativa</div>
              </div>
            ...
            <button className="w-full bg-white border-half rounded-xl px-4 py-3 flex items-center gap-3 hover:border-ink-400 transition-colors text-left mt-2">
              ...
              <div className="flex-1">
                <div className="text-[13.5px] font-medium text-ink-900">Perguntar à Inês (expert local)</div>
                <div className="text-[12px] text-ink-500">Resposta humana em ~2 minutos</div>
              </div>
              <Icon.ArrowRight size={14} className="text-ink-500"/>
            </button>
          </div>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="bg-ink-50 border-half rounded-xl p-3 flex items-start gap-3">
            ...
            <div className="text-[12.5px] text-ink-700">
              Mudanças manuais não invalidam o roteiro. A Voya reorganiza horários e logística sozinha.
            </div>
          </div>
        </div>
      )}
...
// ---------- Add modal ----------
const suggestions = [
  { title: 'Almoço no Bairro do Avillez', place: 'Chiado', dur: '1 h 30', tag: 'comida', vibe: 'autoral' },
  { title: 'Tram 28 + Miradouro',         place: 'Graça',  dur: '2 h',    tag: 'clássico', vibe: 'turístico' },
  { title: 'Tour Berardo · arte moderna', place: 'Belém',  dur: '2 h',    tag: 'arte', vibe: 'contemplativo' },
  { title: 'Tasca em Alfama (Inês)',      place: 'Alfama', dur: '1 h 30', tag: 'comida', vibe: 'íntimo' },
  { title: 'Spa no Memmo',                place: 'Hotel',  dur: '1 h',    tag: 'experiência', vibe: 'respirar' },
];
    ...
    <Modal open={open} onClose={onClose} title={`Adicionar atividade · ${adding.slot}`}>
      <div className="space-y-3">
        ...
        <div className="space-y-2 max-h-[380px] overflow-y-auto">
          ...
          {filtered.length === 0 && (
            <div className="p-6 text-center text-[13px] text-ink-500">Nada encontrado. Tente pedir à Voya pelo chat.</div>
          )}
        </div>
      </div>
    </Modal>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <input readOnly value="voya.app/r/he-lisportu-2026"
                   className="flex-1 h-10 px-3 rounded-lg border hairline bg-white text-[13px] mono text-ink-700"/>
            <Button onClick={() => { navigator.clipboard?.writeText('voya.app/r/he-lisportu-2026'); setCopied(true); setTimeout(()=>setCopied(false),1500); }}
                    variant={copied?'accent':'secondary'} icon={copied?Icon.Check:Icon.Share}>
              {copied ? 'Copiado' : 'Copiar'}
            </Button>
          </div>
          <div className="text-[11.5px] text-ink-500 mt-2 flex items-center gap-1.5"><Icon.Lock size={11}/> Só quem tem o link consegue ver. Não aparece em buscas.</div>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-ink-50 border hairline rounded-xl p-4">
          <div className="text-[13px] text-ink-800">
            Vou criar <span className="font-medium">{days.reduce((s,d)=>s+d.items.length,0)} eventos</span> em {days.length} dias com fuso de <span className="mono">Europe/Lisbon</span>.
          </div>
